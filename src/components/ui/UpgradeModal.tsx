  ...
  return (
    ...
    >
      ...
      <div className="relative bg-white rounded-[20px] shadow-2xl max-w-sm w-full p-6 text-center">
        <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors text-[#9CA3AF]">
        ...
        <Button onClick={handleUpgrade} loading={loading} className="w-full py-3">
          {loading ? "Opening..." : "Upgrade to Premium"}
        </Button>

        {error && <p className="text-red-500 text-sm mt-3">{error}</p>}
        <p className="text-xs text-[#9CA3AF] mt-3">Secure payment powered by Paddle</p>
      </div>
    </div>
  );
}
